...
@section('content')
    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <div class="breadcrumb-title pe-3">{{__('Countries')}}</div>
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    <li class="breadcrumb-item"><a href="{{route($role . '-profile')}}"><i class="bx
                    bx-home-alt"></i></a></li>
                    <li class="breadcrumb-item active" aria-current="page">{{__('Countries list')}}</li>
                </ol>
            </nav>
        </div>
    </div>
            ...
            <div class="table-responsive">
                <div class="ms-auto" style="margin-bottom: 20px">
                    <a href="{{route('countries.create')}}" class="btn btn-primary radius-30 mt-2 mt-lg-0">
                        <i class="bx bxs-plus-square"></i>{{__('Add new Country')}}</a></div>
                        ...
                        <tr id="tr{{$item->id}}" data-id="{{$item->id}}">
                                            ...
                                            <div class="modal-body">
                                                <div class="card-body">
                                                    ...
                                                    <div class="col-lg-5 col-md-5">
                                                        ...
                                                        @forelse($item->restrictions as $restriction)
                                                            <span style="font-weight:  lighter" class="p-3">{{$restriction->name}}</span>
                                                            @empty
                                                        @endforelse
                                                    </div>
                                                    <div class="col-lg-5 col-md-5 mt-2">
                                                        <h5 class="card-title">Prohibitions :</h5>
                                                        @forelse($item->prohibitions as $pro)
                                                            <span style="font-weight:  lighter" class="p-3">{{$pro->name}}</span>
                                                        @empty
                                                        @endforelse
                                                    </div>
                                                </div>
                            ...
                            <td>
                                <div class="d-flex order-actions">
                                    <a href="{{url('countries/'.$item->id.'/edit')}}" class=""  ><i class='bx bxs-edit'></i></a>

                                    <a  class="ms-3 delete" data-bs-toggle="modal" data-url="{{route('countries.destroy', $item->id) }}" data-bs-target="#deleteModal">
